...
//$mcb_paged = get_post_meta($post->ID, "mcb_paged", true);
  //  echo $mcb_paged;

?>
    <div class="fullscreen_block hided" style="background-image: url('https://www.mcolinburch.com/wp-content/uploads/201    4/03/MCB_7266_LR_Edit_1.jpg');">
                        ...
                        <div class="img_block wrapped_img">
	                        <?php
								if ($image['slide_type'] == "image") {
									echo '<a class="featured_ico_link swipebox" rel="gallery-module" href="'. wp_get_attachment_url($image['attach_id']) .'" data-caption="'.$photoCaption.'" title="'.$photoTitleOutput .'">';
                                    $icon = "search-plus";
								} else {
									$set_rel = '';
									...
									echo '<a href="'. $image['src'] .'" class="featured_ico_link swipebox" rel="'. $set_rel .'" title="'.$photoTitleOutput.'" data-caption="'.$photoCaption.'">';
								}									
	                        ?>
                            	<img width="540" height="" src="<?php echo aq_resize(wp_get_attachment_url($image['attach_id']), "540", "", true, true, true); ?>" data-caption="<?php echo $photoCaption ?>" alt="<?php echo $photoTitleOutput ?>">
                                <div class="featured_item_fadder"></div>
                                <div class="gallery_likes gallery_likes_add"><i data-icon-code="icon-<?php echo $icon ?>" class="stand_icon icon-<?php echo $icon ?>"></i></div>
							</a>
                        </div>                    
                    </div>
                </div>      
