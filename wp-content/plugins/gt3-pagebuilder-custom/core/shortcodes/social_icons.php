...
if (is_array($GLOBALS["pbconfig"]['all_available_social_icons_type'])) {
    foreach ($GLOBALS["pbconfig"]['all_available_social_icons_type'] as $socialType => $socialCaption) {
        $gt3_compileShortcodeUI .= "<option value='".$socialType."'>".$socialCaption."</option>";
    }
}
...
<div class='social_icons all_available_font_icons'>
	...
	<ul class='icons_list ".$shortcodeName."_icon'>";
        foreach ($GLOBALS["pbconfig"]['all_available_font_icons'] as $icon) {
            $gt3_compileShortcodeUI .= "<li class='stand_social'><i data-icon-code='" . $icon . "' class='stand_icon " . $icon . "'></i></li>";
        }		
$gt3_compileShortcodeUI .= "
...
shortcodesUI::getInstance()->add('social_icon', array("name" => $shortcodeName, "caption" => "Icon", "handler" => $gt3_compileShortcodeUI));
